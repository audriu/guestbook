...
(def pyramid-points
  [[-0.5 -0.5 0 "#63B132"] [-0.5 0.5 0 "#5881D8"] [0.5 0.5 0 "#90B4FE"] [0.5 -0.5 0 "#91DC47"] [0 0 1 "red"]])
...
(defn gasket-3d []
  (let [camera (js/THREE.PerspectiveCamera. 45 1 1 2000)
        scene (doto (create-scene)
                (add-pyramids 0 0 0 32 "red"))
        tick (fn [] (fly-around-z-axis camera scene))]
    [three-canvas {:width 640 :height 640} camera scene tick]))

(defn things-page []
  [:section.section>div.container>div.content
   [:p "code taken from https://timothypratley.blogspot.com/2017/01/reagent-deep-dive-part-2-lifecycle-of.html"]
   (gasket-3d)])
